...
class BackgroundWorker:
    ...
    async def process_job(self, job_data: Dict[str, Any]):
        ...
        try:
            ...
            if not raw_text:
                job_manager.update_job_status(job_id, JobStatus.FAILED, "Could not extract text from PDF")
                return
            ...
            print(f"📄 Job {job_id}: Generated {total_chunks} chunks")

            # 4. EMBED (Process chunks in batches)
            print(f"🔄 Job {job_id}: Starting batch embedding for {total_chunks} chunks")
            vectors = self.embedder.process_chunks_in_batches(chunks)

            # Update progress to 100% after embedding is complete
