...
import { useGetActivityHeatmapQuery } from "services/analytics/analytics";
import { toHeatmapColumns } from "../../../utils/analytics/heatmapUtils";
...
export const ActivityHeatmapCard = ({ workspaceId, agentId }: ActivityHeatmapCardProps) => {
    const { data: days = [], isFetching } = useGetActivityHeatmapQuery(
        { workspaceId, agentId },
        { skip: !workspaceId || !agentId },
    );
    const heatmapData = useMemo(() => toHeatmapColumns(days), [days]);
