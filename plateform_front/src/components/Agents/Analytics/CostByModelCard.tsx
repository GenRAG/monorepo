...
import { costByModelTotals, fmtCredits, usdToCredits } from "./costUtils";
import { COST_MODELS, PERIOD_DAYS, type Period } from "./types";
...
export const CostByModelCard = () => {
    const [period, setPeriod] = useState<Period>("30j");
    const rows = useMemo(() => baseMetrics.slice(-PERIOD_DAYS[period]), [period]);
    const modelRows = useMemo(() => {
        const totals = costByModelTotals(rows);
        return COST_MODELS.map((model) => ({ name: model.label, value: usdToCredits(totals[model.id] ?? 0) })).sort(
            (a, b) => b.value - a.value,
        );
    }, [rows]);
