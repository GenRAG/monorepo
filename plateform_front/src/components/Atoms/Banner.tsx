...
const Banner = ({ variant, isCloseable, children, buttonText, onClick, size = 'xs', ...props }: RamifyBannerProps) => {
		...
		>
			...
			<HStack w="100%" spacing="12px" align="center" position="relative" zIndex={1} flex="4">
				...
				{(buttonText || isCloseable) && (
					<HStack flex="1" justify="space-between" w="100%" justifyContent="end">
						{buttonText && (
							<Button
								variant="secondary"
								icon={ArrowRight}
								size="sm"
								btnType={isMobile ? 'icon' : buttonText ? 'default' : 'icon'}
						...
						{isCloseable && <Button variant="link" size="sm" btnType="icon" onClick={handleCloseBanner} icon={X} />}
					</HStack>
				)}
			</HStack>
		</Card>
