...
import type { AppNodeData, Task, TaskParam } from "@genrag/workflow";
import { TaskType } from "@genrag/workflow";
import InstructionNode from "components/Molecules/WorkflowPreview/SettingNodes/InstructionNode";
import ModelNode from "components/Molecules/WorkflowPreview/SettingNodes/ModelNode";
import { NodeShape } from "pages/Workspace/Workflow/NodeShape";

const { TaskRegistry, useWorkflowNodes } = WorkflowPackage as any;

interface AppWorkflowNodeProps extends NodeProps {
    onNodeClick?: (nodeId: string) => void;
    isVertical?: boolean;
}

const NodeComponent = memo((props: AppWorkflowNodeProps) => {
    const nodeData = props.data as AppNodeData;
    const task = TaskRegistry[nodeData.type] as Task;
    const isMobile = useAppResponsive({ base: true, lg: false });

    const { handleRemoveChainNode } =
        (
            useWorkflowNodes as unknown as () => {
            ...
            >
                ...
                >
                    <HStack spacing={2} justify="space-between">
                        ...
                        {!isMobile &&
                            (task.isEntryPoint || task.isEndPoint) && (
                                <Badge
                                    fontSize="8px"
                                    fontWeight={700}
