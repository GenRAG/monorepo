...
function NodeHeader({
  ...
  const { deleteElements, getNodes, getEdges, setEdges, setNodes } = useReactFlow();
  ...
  const handleDeleteNode = () => {
    ...
    if (incomingEdge && outgoingEdge) {
      ...
      if (sourceNode && targetNode) {
        ...
        setNodes((prevNodes) =>
          prevNodes.map((node) => {
            if (node.id === targetNode.id || node.position.x >= targetNode.position.x) {
              return {
                ...node,
                position: {
  ...
  return (
    <div className="flex items-center gap-2 p-2">
      <task.icon size={16} />
      <div className="flex justify-between items-center w-full" id={task.id}>
