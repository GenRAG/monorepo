...
export const NodeShape = ({
  ...
  size = 80,
  iconSize = 32,
  backgroundColor = { light: "#FFFFFF", dark: "#34D3A9" },
  borderColor = { light: "#34D3A9", dark: "#E7E7E7" },
  canHover = true,
}: NodeShapeProps) => {
  const [isHovered, setIsHovered] = useState(false);

  const borderColorValue = useColorModeValue(borderColor.light, borderColor.dark);
  const hoverBackgroundColor = useColorModeValue("#F7FAFC", "#2F855A");
  const backgroundColorValue = useColorModeValue(backgroundColor.light, backgroundColor.dark);
  ...
  return (
    ...
    >
      ...
      <Box
        ...
        bg={isHovered && canHover ? hoverBackgroundColor : isSelected ? selectedBackgroundColor : backgroundColorValue}
        transition="all 0.2s"
