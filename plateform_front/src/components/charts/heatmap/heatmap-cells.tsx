...
import { type HeatmapBin, type HeatmapColumn, useHeatmap, useHeatmapInteraction } from "./heatmap-context";
import {
...
function syncHeatmapCellLayerOpacities(params: SyncCellLayerParams) {
    ...
    if (!animateCells) {
        if (isReadyResting) {
            ...
            dataOpacity.set(0);
            pulseOpacity.set(0);
            shimmerOpacity.set(participates && showLoadingCellsLayer ? 0 : HEATMAP_LOADING_BASE_CELL_OPACITY);
        }
        return;
    }
...
const HeatmapMotionCell = memo(function HeatmapMotionCell({
    ...
    const isRevealActive =
        chartPhase === "revealing" && !isLoaded && (revealMode === "fromLoading" || revealMode === "enter");
    ...
    return (
        <motion.g animate={{ scale: readyScale }} style={{ transformOrigin }} transition={HEATMAP_INACTIVE_TRANSITION}>
            <motion.rect
                {...cellProps}
                fill={targetFill}
