...
export function heatmapLevelPatternRenderOptions(style: HeatmapLevelStyle) {
    ...
    return {
        color:
            style.patternColor?.trim() || (preset === "accent" ? "#e879f9" : heatmapPatternStrokeFallback(style.color)),
...
export function buildHeatmapColorScale(levelColors: HeatmapLevelColors): (count: number | null | undefined) => string {
...
export function buildHeatmapFillScale(levelStyles: HeatmapLevelStyles): (count: number | null | undefined) => string {
    return (count: number | null | undefined) => {
        const level = getHeatmapContributionLevel(count ?? 0);
