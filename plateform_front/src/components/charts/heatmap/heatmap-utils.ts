...
const heatmapTooltipMonthFmt = new Intl.DateTimeFormat("fr-FR", {
  month: "long",
});

const heatmapTooltipWeekdayFmt = new Intl.DateTimeFormat("fr-FR", {
  weekday: "long",
});

/** Tooltip header date — e.g. `20 janvier 2026`. */
export function formatHeatmapTooltipDate(date: Date): string {
  const month = heatmapTooltipMonthFmt.format(date);
  return `${date.getDate()} ${month} ${date.getFullYear()}`;
}

/** Tooltip weekday line — e.g. `lundi`. */
export function formatHeatmapTooltipWeekday(date: Date): string {
  const weekday = heatmapTooltipWeekdayFmt.format(date);
  return weekday.charAt(0).toUpperCase() + weekday.slice(1);
}

/** Tooltip contribution line — e.g. `3 requêtes`. */
export function formatHeatmapContributionLabel(
  count: number,
  _date?: Date
): string {
  const word = count === 1 ? "requête" : "requêtes";
  return `${count} ${word}`;
}
