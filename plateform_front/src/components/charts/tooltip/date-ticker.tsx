...
const DateTickerInner = memo(function DateTickerInner({ currentIndex, labels }: Omit<DateTickerProps, "visible">) {
    // Parse labels into month and day parts. Order-agnostic (e.g. en-US "Aug 19"
    // vs fr-FR "19 août") — the numeric token is always the day.
    const parsedLabels = useMemo(() => {
        return labels.map((label, index) => {
            const parts = label.split(" ");
            const day = parts.find((part) => /^\d+$/.test(part)) || "";
            const month = parts.find((part) => !/^\d+$/.test(part)) || "";
