...
import { Link as ReachLink, useLocation, useNavigate, useParams, useSearchParams } from 'react-router-dom';
import {
	Button,
	chakra,
	Flex,
...
const ApplyResetPassword: FC = () => {
	...
	const onSubmit = handleSubmit(({ password }: ApplyResetFormType) => {
		applyResetPassword({ password, token: Number(token), email: email || '' })
			.unwrap()
			.then(() => {
				toast({
					status: 'success',
