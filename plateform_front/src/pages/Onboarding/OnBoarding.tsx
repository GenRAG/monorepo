...
const Onboarding: React.FC = () => {
  ...
  const validateAndGoNextRef = useRef<(() => Promise<void>) | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    if (prevStep !== currentStep) {
      ...
      // Scroll the main onboarding container to top
      setTimeout(() => {
        const mainContainer = document.querySelector('[data-onboarding-container]') as HTMLElement;
        if (mainContainer) {
      ...
      // Réinitialiser la fonction de validation lors du changement d'étape
      validateAndGoNextRef.current = undefined;
      return () => clearTimeout(timer);
    }
  }, [currentStep]); // Seulement réagir aux changements de currentStep
  ...
  const StepperContent = () => (
    <Stepper
      ...
      variant={colorMode === 'dark' ? currentDarkTheme.colorScheme : currentDarkTheme.colorScheme}
    >
      {stepsConfig.map((step, index) => (
        ...
        >
          <StepIndicator
            flexShrink={0}
            bg={index <= currentStep ? colorMode === 'dark' ? currentDarkTheme.primary : currentDarkTheme.primary : colorMode === 'dark' ? 'rgba(255, 255, 255, 1)' : 'white'}
            border="none"
            transition="all 0.3s ease-in-out"
            transform={index === currentStep ? 'scale(1.1)' : 'scale(1)'}
            boxShadow={index === currentStep ? `0 0 0 4px ${colorMode === 'dark' ? currentDarkTheme.rgba.primary20 : currentDarkTheme.rgba.primary20}` : 'none'}
            className={index === justActivatedStep ? 'step-indicator-active' : ''}
          >
            <StepStatus
              complete={
                <Circle
                  size="45px"
                  mr="1px"
                  bg={colorMode === 'dark' ? currentDarkTheme.completeColor : currentDarkTheme.completeColor}
                  className={index === justCompletedStep ? 'step-indicator-animation' : ''}
                >
              ...
              active={
                <Icon
                  ...
                  boxSize={5}
                  className={index === justActivatedStep ? 'step-indicator-animation' : ''}
                />
              }
            />
          </StepIndicator>
          <Box flex={1} ml={4} minW={0}>
            ...
            <StepDescription>
              <Text
                fontSize="sm"
                color={colorMode === 'dark' ? 'white' : 'grey.600'}
                mt={1}
              >
                {step.description}
              </Text>
          ...
          >
            ...
            {index < currentStep && index !== justCompletedStep && (
              <Box
                ...
                style={{
                  '--separator-fill-color': colorMode === 'dark' ? currentDarkTheme.hex.primary : currentDarkTheme.hex.primary,
                  height: '100%',
                  opacity: 1,
  ...
  return (
    <Stack h="100vh" bg={colorMode === 'dark' ? 'grey.900' : 'grey.50'} spacing={0}>
      <HStack
      ...
      >
        {showStepper && (
          <VStack
            ...
            border="1px solid"
            position="relative"
            borderColor={colorMode === 'dark' ? currentDarkTheme.rgba.primary20 : '#acacac81'}
          >
            <Image
              src={colorMode === 'dark' ? OnBoardingBackgroundBlack : OnBoardingBackground}
              position="absolute"
              bottom="0"
              right="0"
              maxW="100%"
        ...
        <Drawer isOpen={isOpen} placement="left" onClose={onClose}>
          ...
          >
            <DrawerCloseButton color={colorMode === 'dark' ? 'white' : 'grey.900'} bg={colorMode === 'dark' ? 'grey.800' : 'white'} />
            <DrawerHeader
              borderBottomWidth="1px"
              bg={colorMode === 'dark' ? 'grey.800' : 'white'}
              borderColor={colorMode === 'dark' ? currentDarkTheme.rgba.primary20 : 'grey.200'}
              color={colorMode === 'dark' ? 'white' : 'grey.900'}
            >
              Navigation
            </DrawerHeader>
        ...
        >
          ...
          >
            <Stack w="100%" spacing={6}>
              <Text
                fontSize="sm"
                color={colorMode === 'dark' ? currentDarkTheme.primary : currentDarkTheme.primary}
                fontWeight="semibold"
              ...
              >
                <CurrentStepComponent
                  ...
                  registerValidateAndGoNext={registerValidateAndGoNext}
                />
              </Box>
            </Stack>
            <StepFooter
              ...
              showReassuringMessage={currentStepConfig.id === 'knowledge'}
            />
          </VStack>
        </VStack>
      </HStack>
