...
const StorageBar: React.FC<{
    ...
    console.log(byType, freeBytes, quotaBytes);
...
export const StorageOverviewPanel: React.FC<StorageOverviewPanelProps> = ({
    ...
    const [quotaVal, _setQuotaVal] = useState<number>(DEFAULT_QUOTA_GB);
    const [inputVal, _setInputVal] = useState<string>(String(DEFAULT_QUOTA_GB));
    const [unit, _setUnit] = useState<"GB" | "MB">("GB");
    ...
    const { byType, totalUsed, freeBytes, usedPct } = useMemo(() => {
        ...
        documents.forEach((doc) => {
            sums[getTypeKey(doc.type)] += doc.size;
        });
        console.log(sums);
